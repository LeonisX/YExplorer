...
const
  ...
  MPH_CCONV_MAC: TMPHCharConv = (
    ...
    (#$00, #$01, #$02, #$03, #$04, #$05, #$06, #$07, 
     ...
     #$C4, #$C5, #$AB, #$C9, #$D1, #$F7, #$DC, #$E1, 
     #$E0, #$E2, #$E4, #$E3, #$AC, #$B0, #$AA, #$F8,
     #$D5, #$CE, #$C3, #$CF, #$D3, #$D4, #$D2, #$DB,
     #$DA, #$DD, #$F6, #$F5, #$FA, #$F9, #$FB, #$FC,
     #$A0, #$C1, #$A2, #$A3, #$DF, #$B4, #$B6, #$A4, 
     #$C6, #$A9, #$BB, #$C7, #$C2, #$AD, #$A8, #$FF,
     #$A1, #$B1, #$B2, #$B3, #$A5, #$B5, #$A6, #$B7,
     #$B8, #$B9, #$BC, #$C8, #$BA, #$BD, #$CA, #$C0,
     #$CB, #$E7, #$E5, #$CC, #$80, #$81, #$AE, #$82, 
     #$E9, #$83, #$E6, #$E8, #$ED, #$EA, #$EB, #$EC,
     #$D0, #$84, #$F1, #$EE, #$EF, #$CD, #$85, #$D7, 
     #$AF, #$F4, #$F2, #$F3, #$86, #$D9, #$DE, #$A7,
     #$88, #$87, #$89, #$8B, #$8A, #$8C, #$BE, #$8D, 
     #$8F, #$8E, #$90, #$91, #$93, #$92, #$94, #$95,
     #$F0, #$96, #$98, #$97, #$99, #$9B, #$9A, #$D6, 
...
begin
  case TType of
    ...
    tkBCD	: if aCount > 0 then
                    for LIntLoop := 0 to Pred(aCount) do
                      bBuffer[LIntLoop] := MPH_CCONV_BCD38[cctToAnsi][Ord(aBuffer[LIntLoop])];
    tkCustom	: if aCount > 0 then
                    for LIntLoop := 0 to Pred(aCount) do
                      bBuffer[LIntLoop] :=  MPHCustomCharConv[cctToAnsi][Ord(aBuffer[LIntLoop])];
  end;
end;
...
function TCustomMPHexEditor.GetAnsiDecode : Boolean;
begin
  Result := (not FUnicodeCharacters) and (FTranslation in [tkAsIs, tkDOS8]);
end;
...
procedure StringOut(Handle : THandle; const X, Y : Integer; aRect : TRect; const Str: WideString; TabSize : Integer; AnsiDecode : Boolean);
var
  S : WideString;
begin
  ...
  if Win32Platform = VER_PLATFORM_WIN32_NT then
    ExtTextOutW(Handle, X, Y, ETO_CLIPPED or ETO_OPAQUE, @aRect, PWideChar(S), Length(S), NIL)
  else
    ExtTextOutA(Handle, X, Y, ETO_CLIPPED or ETO_OPAQUE, @aRect, PAnsiChar(AnsiString(S)), Length(S), NIL);
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (LWChrOutput < #256) and ({$IFNDEF DELPHI2006UP}Char{$ENDIF}(LWChrOutput) in FMaskedChars) then
      ...
      begin
        ...
        begin
          if (LIntCurCol < LIntLastCol) and (LIntCurCol <> FIntLastHexCol) and
        ...
        begin
          ...
          if FUnicodeCharacters then
            while (Canvas.Font.Size > 1) and 
                   GetTextExtentPoint32W(Canvas.Handle, @LWChrOutput, 1, 
                   LrecSize) and (LRecSize.cx > (LRect2.Right - LRect2.Left)) do
              Canvas.Font.Size := Canvas.Font.Size - 1;
// !!!!!!!!!
// {Incorrect} 
// {It is corrected by me. DSoft}
//          ExtTextOutW(Handle, Left, Top, ETO_CLIPPED or ETO_OPAQUE, @LRect2, @LWChrOutput, 1, nil);
// !!!!!!!!!
	  StringOut(Handle, Left, Top, LRect2, WideString(LWChrOutput), 8, GetAnsiDecode);
    ...
    begin
      ...
      if FShowPositionIfNotFocused and (LIntCurRow = Row) and (Col = LIntCurCol) then
      begin
        with LRctWhere do
          if FFocusFrame then
            Canvas.DrawFocusRect(Rect(Left, Top, Left + FCharWidth, Bottom - 1))
