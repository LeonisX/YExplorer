  ...
  while (keepReading) do
    begin
      ...
      case DTA.ChunkIndex(s) of
       ...
       10: ReadTNAM; //названи€+тайлы
       11: ReadTGEN; //встречаетс€ в германском образе. „то такое - не знаю.
       12: keepReading:=false;
      else
       begin
        showmessage('Ќеизвестна€ секци€: '+s);
        keepReading:=false;
       end;
      end;
  end;
...
begin
  ...
  if CAUXCB.Checked then
    begin
    //надо расшифровать
    showmessage('ќбработка CAUX пока не поддерживаетс€!!!');
    seek(SrcFile,filepos(SrcFile)+size);
    end else
       seek(SrcFile,filepos(SrcFile)+size);
end;
...
begin
  ...
  if CHWPCB.Checked then
    begin
    //надо расшифровать
    showmessage('ќбработка CHWP пока не поддерживаетс€!!!');
    seek(SrcFile,filepos(SrcFile)+size);
    end else
       seek(SrcFile,filepos(SrcFile)+size);
end;
...
begin
  ...
  if CHARCB.Checked then
    ...
    showmessage('ќбработка CHAR пока не поддерживаетс€!!!');
    seek(SrcFile,filepos(SrcFile)+size);
    end else
       seek(SrcFile,filepos(SrcFile)+size);
end;
...
begin
  ...
  if PUZ2CB.Checked then
    begin
    //2 байта - не используетс€ (00 00)
    //2 байта - номер паззла
    //4 байта IPUZ
    //2 байта - длина паззла (до конца текущего)
    showmessage('ќбработка PUZ2 пока не поддерживаетс€!!!');
    seek(SrcFile,filepos(SrcFile)+size);
    end else
       seek(SrcFile,filepos(SrcFile)+size);
end;
