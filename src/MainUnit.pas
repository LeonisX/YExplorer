...
s:string;
begin
//  if not OpenDialog1.Execute then Exit; // ≈сли файл не выбран, то выходим
//  AssignFile(fx,Form1.OpenDialog1.FileName); // —в€зываем файл с переменной f
  AssignFile(fx,'D:\YodaStories\PatchWise.bak\yodesk.dta'); // —в€зываем файл с переменной f
  try
    Reset(fx,1); // ќткрываем файл дл€ чтени€
  except
    ShowMessage('Ќевозможно открыть "'+OpenDialog1.FileName+'"!');
    Exit;
  end;
...
var
  Sentence, { ¬ыводимый текст }
  CurWord: string; { “екущее выводимое слово }
  SpacePos, { ѕозици€ первого пробела }
  CurX, { ’-координата 'курсора' }
  CurY: Integer; { Y-координата 'курсора' }
  EndOfSentence: Boolean; { ¬еличина, указывающа€ на заполненность €чейки }
  hGrid: TStringGrid;
  k,n:integer;
begin

  { »нициализируем шрифт, чтобы он был управл€ющим шрифтом }
  hGrid:=(Sender as TStringGrid);
  hGrid.Canvas.Font := Font;

  with hGrid.Canvas do
  begin
    { ≈сли это фиксированна€ €чейка, тогда используем фиксированный цвет }
    ...
    { –исуем подложку цветом €чейки }
    Rectangle(Rect.Left, Rect.Top, Rect.Right, Rect.Bottom);
  end;

  { Ќачинаем рисование с верхнего левого угла €чейки }
  CurX := Rect.Left;
  CurY := Rect.Top;

  { «десь мы получаем содержание €чейки }
  Sentence := hGrid.Cells[ACol, ARow];
  n:=0;

  { дл€ каждого слова €чейки }
  EndOfSentence := FALSE;
  while (not EndOfSentence) do
  begin
    { дл€ получени€ следующего слова ищем пробел }
    ...
    begin
      ...
      { получаем остальную часть предложени€ }
    ...
    with hGrid.Canvas do
    begin
      { если текст выходит за границы €чейки }
